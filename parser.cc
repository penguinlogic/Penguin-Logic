...
bool parser::readin(void)
{
	try {
		...
		cout <<
		    "Cannot run simulator. Use './logsim --help' to see required syntax."
		    << endl;
		return false;
	}
}
...
/***************************************************************************************/
class uintoutofboundsex:public exception {
 public:
	virtual const char *what() const throw() {
		return "Exception: Number of inputs must be between 1 and 16";
}} uintoutofboundsex_i;
			...
			if (cursym.get_name_id() == nmz->cvtname("SWITCH")) {	//i.e. devname is SWITCH
				...
				if (cursym.get_syntaxvalue() == dash) {	// i.e. symbol is '-'
					...
					if (cursym.get_syntaxvalue() == initialvalue) {	//i.e. devswitch is 'initialvalue'
						smz->getsymbol(cursym);
						if (cursym.get_type() == Uint && (cursym.get_uint() == 0 || cursym.get_uint() == 1)) {	// i.e. symbol is a uint but is also a bool
							devkind_var = aswitch;
							variant_var =
							    cursym.get_uint();
...
class unameex:public exception {
 public:
	virtual const char *what() const throw() {
		return "Exception: expected a user-defined name";
}} unameex_i;
...
class makeconex:public exception {
 public:
	virtual const char *what() const throw() {
		return
		    "Exception: Connection failed - input pin/device mismatch";
}} makeconex_i;
...
void parser::connrule(void)	// throws connruleex if finds invalid connrule
{
	try {
		...
		if (cursym.get_syntaxvalue() == dot) {	// cursym is '.'
			...
			outname(outp);
			if (outp != blankname
			    && dtype != netz->finddevice(odev)->kind) {
				errcount++;
				throw badoutpex_i;
			}
			...
			if (cursym.get_syntaxvalue() == dot) {	// cursym is '.'
				...
				if (cursym.get_syntaxvalue() == semicolon) {	// cursym is ';'
					for (unsigned int i = 0;
					     i < idev_conn_list.size(); i++) {
						if (idev == idev_conn_list[i]
						    && inp == inp_conn_list[i]) {
							errcount++;
							throw
							    idevalreadyusedex_i;
	...
	try {
		...
		if (cursym.get_syntaxvalue() == equals) {	// we have an '='
			...
			if (cursym.get_syntaxvalue() == semicolon) {	// we have a ';'
				dmz->makedevice(devkind_var, did_var,
						variant_var, wvvector_var, ok);
