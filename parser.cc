...
//    cout << "It worked!" << endl;
	...
	return true;
      ...
      {
	cout << "Could not parse definition file: " << errcount <<
	  " errors found" << endl;
	return false;
      }
  }
...
/***************************************************************************************/
void
parser::inname (name & iname)	// throws innameex if not given a valid inname
{
  try
  {
    if (cursym.get_type () != Inname)
      {
	errcount++;
	throw innameex_i;
      }				// didn't get an inname
    else
...
/***************************************************************************************/
void
parser::outname (name & oname)	// throws outnameex if not given a valid outname
{
  try
  {
    if (cursym.get_type () != Outname)
      {
	errcount++;
	throw outnameex_i;
      }				// didn't get an outname
    else
...
void
parser::device (devicekind & devkind_var, int &variant_var)	// throws deviceex if finds invalid device
{
  try
  {
    if (cursym.get_type () == Devname)
      {				//i.e. if symbol is a 'devname'
	 ...
	 */
	if (cursym.get_name_id () == nmz->cvtname ("CLOCK"))
	  {			//i.e. devname is CLOCK
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
		smz->getsymbol (cursym);
		if (cursym.get_syntaxvalue () == period)
		  {		//i.e. devswitch is '-period'
		    smz->getsymbol (cursym);
		    if (cursym.get_type () == Uint)
		      {		// i.e. symbol is a uint
			devkind_var = aclock;
	...
	if (cursym.get_name_id () == nmz->cvtname ("SWITCH"))
	  {			//i.e. devname is SWITCH
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
		smz->getsymbol (cursym);
		if (cursym.get_syntaxvalue () == initialvalue)
		  {		//i.e. devswitch is 'initialvalue'
		    smz->getsymbol (cursym);
		    if (cursym.get_type () == Uint &&
			(cursym.get_uint () == 0 || cursym.get_uint () == 1))
		      {		// i.e. symbol is a uint but is also a bool
			devkind_var = aswitch;
			variant_var = cursym.get_uint ();
	...
	if (cursym.get_name_id () == nmz->cvtname ("AND"))
	  {			//i.e. devname is AND
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
		smz->getsymbol (cursym);
		if (cursym.get_syntaxvalue () == numinputs)
		  {		//i.e. devswitch is 'numinputs'
		    smz->getsymbol (cursym);
		    if (cursym.get_type () == Uint)
		      {		// i.e. symbol is a uint
			devkind_var = andgate;
	...
	if (cursym.get_name_id () == nmz->cvtname ("NAND"))
	  {			//i.e. devname is NAND
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
	...
	if (cursym.get_name_id () == nmz->cvtname ("OR"))
	  {			//i.e. devname is OR
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
	...
	if (cursym.get_name_id () == nmz->cvtname ("NOR"))
	  {			//i.e. devname is NOR
	    smz->getsymbol (cursym);
	    if (cursym.get_syntaxvalue () == dash)
	      {			// i.e. symbol is '-'
	...
	if (cursym.get_name_id () == nmz->cvtname ("DTYPE"))
	  {			//i.e. devname is DTYPE
	    devkind_var = dtype;
	    return;		// we have a valid DTYPE
	  }

	if (cursym.get_name_id () == nmz->cvtname ("XOR"))
	  {			//i.e. devname is XOR
	    devkind_var = xorgate;
...
/**************************************************************************************/
void
parser::uname (name & did_var)	// throws unameex if not given a valid uname
{
  try
  {
    if (cursym.get_type () != Uname)
      {
	errcount++;
	throw unameex_i;
      }				// didn't get a uname
	...
	for (unsigned int i = 0; i < uname_list.size (); i++)
	  {
	    if (cursym.get_name_id () == uname_list[i])
	      {
...
/***************************************************************************************/
void
parser::monrule (void)		// throws monruleex if finds invalid monrule
{
  try
  {
    name dev;
    ...
    uname (dev);
    smz->getsymbol (cursym);
    if (cursym.get_syntaxvalue () == dot)
      {				// cursym is '.'
	smz->getsymbol (cursym);
	outname (outp);
	smz->getsymbol (cursym);
      }
    if (cursym.get_syntaxvalue () == semicolon)
      {				// cursym is ';'
	...
	if (0 == valid_dev_count)
	  {
	    errcount++;
	    throw undefunameex_i;
	  }			//we found a device not initialised in DEVICES
	if (0 == errcount)
	  {
  ...
  {
    smz->print_err (e.what ());
    while (cursym.get_syntaxvalue () != semicolon
	   && cursym.get_syntaxvalue () != endfile)
      {				// iterates while cursym is not ';'
	smz->getsymbol (cursym);
	//throw parseex_i;
      }
  }
...
void
parser::connrule (void)		// throws connruleex if finds invalid connrule
{
  try
  {
    name odev, idev, inp;
	...
	smz->getsymbol (cursym);
	outname (outp);
	if (outp != blankname && dtype != netz->finddevice (odev)->kind)
	  {
	    errcount++;
	    throw badoutpex_i;
	  }
    ...
    if (cursym.get_syntaxvalue () == rarrow)
      {				// cursym is '>'
	smz->getsymbol (cursym);
	uname (idev);
	smz->getsymbol (cursym);
	if (cursym.get_syntaxvalue () == dot)
	  {			// cursym is '.'
		...
		for (unsigned int i = 0; i < idev_conn_list.size (); i++)
		  {
		    if (idev == idev_conn_list[i] && inp == inp_conn_list[i])
		      {
			errcount++;
			throw idevalreadyusedex_i;
		      }
		  }
	  ...
	  {
	    errcount++;
	    throw rulesymdotex_i;
	  }			// we expected a '.'
      }
    else
      {
	errcount++;
	throw rulesymrarrowex_i;
      }				// we expected a '>'
  }
  catch (exception & e)
  {
...
public:
  ...
  {
    return
      "Exception: Duplicate device name. Subsequent devices will not be created";
  }
}
dupunameex_i;
/***************************************************************************************/
void
parser::devrule (void)		// throws devruleex if finds invalid devrule
{
  try
  {
    ...
    smz->getsymbol (cursym);
    if (cursym.get_syntaxvalue () == equals)
      {				// we have an '='
	smz->getsymbol (cursym);
	uname (did_var);
	for (unsigned int i = 0; i < uname_list.size (); i++)
	  {
	    if (did_var == uname_list[i])
	      {
		errcount++;
		throw dupunameex_i;
	      }			// we found a duplicate uname
	  }
...
/***************************************************************************************/
void
parser::section (void)		// throws sectionex if finds invalid section
{
  try
  {
    smz->getsymbol (cursym);
    if (cursym.get_syntaxvalue () == endfile)
      return;
    if (cursym.get_syntaxvalue () == DEVICES)
      {				// cursym is 'DEVICES'
	if (DEVICES != section_var.get_syntaxvalue ())
	  {
	    errcount++;
	    throw sectionorderex_i;
	  }			// check DEVICES comes first
	smz->getsymbol (cursym);
	if (cursym.get_syntaxvalue () == lbrak)
	  {			// cursym is '{' 
	    smz->getsymbol (cursym);
	    while (cursym.get_syntaxvalue () != rbrak
		   && cursym.get_syntaxvalue () != endfile)
	      {			// i.e. while inside braces
		devrule ();
		smz->getsymbol (cursym);
	      }
	  }
    ...
    else if (cursym.get_syntaxvalue () == CONNECTIONS)
      {				// cursym is 'CONNECTIONS'
	if (CONNECTIONS != section_var.get_syntaxvalue ())
	  {
	    errcount++;
	    throw sectionorderex_i;
	  }			// check CONNECTIONS comes second
	smz->getsymbol (cursym);
	if (cursym.get_syntaxvalue () == lbrak)
	  {			// cursym is '{'
	    smz->getsymbol (cursym);
    ...
    else if (cursym.get_syntaxvalue () == MONITOR)
      {				// cursym is 'MONITOR'
	if (MONITOR != section_var.get_syntaxvalue ())
	  {
	    errcount++;
	    throw sectionorderex_i;
	  }			// check MONITOR comes last 
	smz->getsymbol (cursym);
	if (cursym.get_syntaxvalue () == lbrak)
	  {			// cursym is '{'
	    smz->getsymbol (cursym);
