...
scanner::scanner(names * names_mod, const char *defname, bool & ok)
{
	...
	if (!inf) {
		ok = false;
		cout << "Error: Unable to open file '" << defname << "'" <<
		    endl;
	} else {
		ok = true;
...
void scanner::skipcomments()
{
	if (current.ch == '/') {
		...
		if (current.ch == '/') {
			...
			while (current.pos != current.line.size()
			       && !inf.eof());
		}
...
void scanner::getnumber(int &number, vector < int >&wvform)
{
	number = 0;
	string str;
	...
	if (inf.eof())		//End of file reached, endfile symbol returned
	{
		...
		if (idstr == "DEVICES")
			...
			s.set_parameters(Devswitch, waveform, -1, -1);
			 ...
			 || idstr == "SIGGEN")
			s.set_parameters(Devname, novalue, -1,
					 nmz->lookup(idstr));
			 ...
			 || idstr == "I15" || idstr == "I16")
			s.set_parameters(Inname, novalue, -1,
					 nmz->lookup(idstr));

		else if (idstr == "Q" || idstr == "QBAR")
			s.set_parameters(Outname, novalue, -1,
					 nmz->lookup(idstr));

		else
			s.set_parameters(Uname, novalue, -1,
					 nmz->lookup(idstr));
	}
...
{
	cout << "Error in line " << last.linenum << ", at character " << last.
	    pos << ":" << endl;
